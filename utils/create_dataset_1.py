...
class_names = ['stand', 'walk', 'run', "lay", 'eat']  # label.
class_dit = {
    "stand":1,
    "walk":2,
    "run":3,
...
video_folder = './animal/fuse_video/deer/lay'
annot_file = './animal/fuse_video/deer/deer_lay.csv'
...
def create_csv(folder):
    ...
    for fil in list_file:
        ...
        total_frame = total_frame + frames_count
        video = np.array([fil] * frames_count)
        frame = np.arange(1, frames_count + 1)
        # label = np.array([0] * frames_count)
        get_label = class_dit[fil.split("_")[1]]
        label = np.array([get_label] * frames_count)
